...
with form:
    ...
    st.markdown(
        ...
        - You are busy about **{pct_busy_fmttd}** of the week, occupied {weekly_time_text(weekly_total)} of the week.
        - You are left with **{f":red-background[{weekly_time_text(free_time)}]" if weekly_total > 168 else weekly_time_text(free_time)}** of free time each week.
    """
    )

    if weekly_total > max_weekly:
        st.error(
            f"Your weekly inputs are nonsensical. There are only {max_weekly} hours in a week! Readjust."
        )
...
with viz:
    ...
    def work_hour_map():
        avg_workday_hours = 0 if len(workdays) == 0 else weekly_work / len(workdays)
        workdays_long = [weekday_dict.get(day_abbr) for day_abbr in workdays]
    ...
    df["Daily Total"] = df.Sleep + df.Work + df.Extra
    df["Free Time"] = 24 - df["Daily Total"]

    # Calculate yearly sleep totals
    yr_sleep = df.Sleep.sum()
    yr_sleep_avg = 7 * len(df)
    yr_sleep_fmttd = f"~ {yr_sleep:,.0f}"
    sleep_delta_fmttd = f"{round(yr_sleep - yr_sleep_avg, 1):,.0f}"

    # Calculate yearly work totals
    yr_work_raw = df.Work.sum()
    yr_offdays = 0 if len(workdays) == 0 else (holidays * work) + (vacation * work)
    ...
    work_delta_fmttd = f"{round(yr_work - yr_work_avg, 1):,.0f}"
    ...
    pie_df = pd.DataFrame(
        {
            "Category": ["Free Time", "Sleeping", "Working", "Extracurriculars"],
            "Hours": [
                yr_free_time,
                yr_sleep,
                yr_work,
    ...
    pie_df_wide = pd.DataFrame(
        {
            "Free Time": [yr_free_time],
            "Sleeping": [yr_sleep],
            "Working": [yr_work],
            "Extracurriculars": [yr_extra],
            "Max Time in Year": [yr_max_time],
        }
    )
    ...
    with st.container(border=True) as pie_chart:
        ...
        fig.update_traces(
            ...
            textfont_size=14,
            marker=dict(line=(dict(color="#ffffff", width=1.25))),
        )
        fig.update_layout(
    ...
    with st.expander("See dates breakdown"):
        st.caption(
            "Here, you can see how hours are broken down by day across the current year. Although you estimated daily and weekly times on the last screen, there are not 52 weeks perfectly in a year, This table outlines how the hours get distributed across selected working days for this year and then totals them up. So you can be sure that the yearly estimate is fairly close, even if you work M-F and the work week is cut off on Wed. for the current year."
        )
    ...
    with col1:
        st.metric(
            ...
            border=True,
            help="Value below the main metric is the hours of sleep above/below the national yearly average. If metric is green, you sleep X amount more than the U.S. average. If red, then below.",
        )
    with col2:
        st.metric(
            "Yearly Work",
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.metric("Yearly Free Time %", f"{yr_free_time_pct}%", border=True)
    with col2:
        st.metric("Working Days in Year", workdays_in_year, border=True)
...
##### Issues
Email them to me at codyaholmes@outlook.com. In a future update, I'll put in a short form.

##### Purpose
The reason this app came about was because I wanted to get my church brothers and sisters to understand they have much more free time than they think they do. With this free time, they have the ability to read their Bible, memorize verses, or serving others.
                
However, the analysis here is general enough that it could apply to anyone, religious and non-religous alike. Most people _convince themselves_ they don't have time to train for that promotion or that they don't have the time to call their loved ones. But you do. And time is our most valuable asset. It's important you understand where it's all going!

> For everything there is a season, and a time for every matter under heaven. -Ecclesiastes 3:1

If you like this app and would like to buy me a coffee, don't. Use that money to bless someone else instead. 🙂
"""
    )
